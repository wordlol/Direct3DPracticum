...
private:
	...
	private:
		...
		static constexpr const wchar_t* wndClassName = L"Direct3DPracticum"; //имя файла в диспечере задач ну или имя нашего класса
...
public:
	Window(int width, int height, const wchar_t* name)
	{
		...
		wr.left = 100;//Указывает x-координат верхнего левого угла прямоугольника
		wr.right = width + wr.left;//Указывает x-координат нижнего правого угла прямоугольника
		wr.top = 100;//Указывает y-координат верхнего левого угла прямоугольника
...
Window::WinClass::WinClass():hInst(GetModuleHandle(nullptr))
{
	...
	wc.hIcon = nullptr;// Дескриптор значка класса. Этот элемент должен быть дескриптором ресурса значка
	...
	wc.hIconSm = nullptr; // Дескриптор небольшого значка, связанного с классом окна
...
{
	if (msg == WM_NCCREATE) //сообщенеие о создании окна
	{ 
